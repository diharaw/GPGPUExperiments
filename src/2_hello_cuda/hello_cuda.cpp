...
void compileFileToPTX(char *filename, char **ptxResult, size_t *ptxResultSize)
{
...
int main()
{
	...
	compileFileToPTX("parallel_add.cu", &ptx, &size);
	CUmodule mod = loadPTX(ptx);

	CUfunction func;

	cuModuleGetFunction(&func, mod, "parallel_add");
	...
	float cpuOutput[numElements] = { 9.0f, 9.0f, 9.0f, 9.0f, 9.0f, 9.0f, 9.0f, 9.0f, 9.0f, 9.0f };

	CUresult result;

	// Buffers (simply pointers in CUDA)
	CUdeviceptr d_X, d_Y, d_Output;

	std::cout << "\ncuMemAlloc\n" << std::endl;

	result = cuMemAlloc(&d_X, sizeof(float) * numElements);
	std::cout << getCudaDrvErrorString(result) << std::endl;

	result = cuMemAlloc(&d_Y, sizeof(float) * numElements);
	std::cout << getCudaDrvErrorString(result) << std::endl;

	result = cuMemAlloc(&d_Output, sizeof(float) * numElements);
	std::cout << getCudaDrvErrorString(result) << std::endl;

	std::cout << "\ncuMemcpyHtoD\n" << std::endl;

	result = cuMemcpyHtoD(d_X, &cpuArrayX[0], sizeof(float) * numElements);
	std::cout << getCudaDrvErrorString(result) << std::endl;

	result = cuMemcpyHtoD(d_Y, &cpuArrayY[0], sizeof(float) * numElements);
	std::cout << getCudaDrvErrorString(result) << std::endl;

	void* args[] = { (void*)&d_X, (void*)&d_Y, (void*)&d_Output };
	...
	std::cout << "\ncuLaunchKernel\n" << std::endl;
	result = cuLaunchKernel(func, grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, 0, &args[0], 0);
	std::cout << getCudaDrvErrorString(result) << std::endl;
	...
	std::cout << "\ncuMemcpyDtoH\n" << std::endl;
	result = cuMemcpyDtoH(&cpuOutput[0], d_Output, sizeof(float) * numElements);
	std::cout << getCudaDrvErrorString(result) << std::endl;
